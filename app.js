$(document).ready(function() {
	...
	var insertData = function(arr) {
		var tpl = '<div>' + 'User Info: <ul>' + '<li>First name: <span class="js-first">none</span></li>' + '<li>Last name: <span class="js-last">none</span></li>' + '</ul>' + '<hr>' + '</div>';
	...
	$('body').on('click', '.btn-success', function(e) {
      ...
      return $.ajax({
        ...
        console.error(err);
        window.alert('OOOPS! Major fail!');
      });
    });
 });

	// $('body').on('submit', '.btn-success', function(ev) {
	// 	ev.preventDefault();
	// 		var userName = $('.js-name').val();
	// 		var userJob = $('.js-job').val();
